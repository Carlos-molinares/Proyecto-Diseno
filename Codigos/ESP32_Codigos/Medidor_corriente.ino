...
// Definición de los nuevos pines
#define ENABLE  14   // Pin de entrada ENABLE
#define OUT_ENABLE 12   // Pin de salida OUT_ENABLE
#define OUT_mA 13    // Pin de salida OUT_mA
...
void setup() {
  ...
  pinMode(ENABLE, INPUT);  // Pin de entrada ENABLE
  pinMode(OUT_ENABLE, OUTPUT);  // Pin de salida OUT_ENABLE
  pinMode(OUT_mA, OUTPUT);  // Pin de salida OUT_mA
...
void loop() {
  // Verificar el estado de ENABLE y controlar OUT_ENABLE
  if (digitalRead(ENABLE) == HIGH) {
    digitalWrite(OUT_ENABLE, HIGH);  // Si ENABLE está en 1, OUT_ENABLE estará en alto
  } else {
    digitalWrite(OUT_ENABLE, LOW);   // Si ENABLE está en 0, OUT_ENABLE estará en bajo
  }
...
// Verificar el estado de modo_mA y controlar OUT_mA
  digitalWrite(OUT_mA, modo_mA ? HIGH : LOW);  // Si modo_mA es true, OUT_mA estará en alto, si es false estará en bajo
